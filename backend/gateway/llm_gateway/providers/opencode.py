"""OpenCode provider adapter for the reusable LLM gateway boundary.

Provider-specific HTTP/catalog logic is isolated here but the implementation
is gateway-owned: it depends only on ``gateway.llm_gateway.contracts`` and
``httpx``, with no DocMind imports.  Behavior is preserved exactly from
``app.services.llm.providers.opencode``.
"""
