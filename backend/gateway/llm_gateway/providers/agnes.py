"""Agnes provider adapter for the reusable LLM gateway boundary.

Provider-specific HTTP logic is gateway-owned: it depends only on
``gateway.llm_gateway.contracts`` and ``httpx``/``asyncio``, with no DocMind
imports. Behavior is preserved exactly from
``app.services.llm.providers.agnes``.
"""
