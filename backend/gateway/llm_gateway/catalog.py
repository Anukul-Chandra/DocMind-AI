"""Reusable catalog discovery infrastructure for the LLM gateway boundary.

This module owns the authoritative free-model discovery that was previously
embedded in ``app.services.llm.models_dev_catalog``.  The implementation is
provider-agnostic: it fetches the single large ``https://models.dev/api.json``
document and identifies free models by zero pricing (``cost.input == 0 AND
cost.output == 0``), which is the same signal OpenCode itself uses.  Provider-
specific layers (Agnes, OpenCode) supply only the ``provider`` key (e.g.
``"agnes"`` / ``"opencode"``) and their own curation/error mapping.

Behavior is preserved exactly from the original DocMind module, including:

- TTL cache with lazy refresh, stale-while-revalidate, thread-safe
  in-flight deduplication, and injectable clock/client for deterministic tests.
- ``parse_provider_free_models`` malformed-payload degradation to ``[]``.
- ``_is_free_cost`` strict cost validation (both input and output must be
  numeric zero).
"""
